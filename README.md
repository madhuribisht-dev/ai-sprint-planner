...
As a landlord applying for a buy-to-let mortgage, I want to enter my desired loan amount, property value, and preferred repayment tenure so that I can instantly see my loan-to-value ratio and indicative eligibility before committing to a full application.
As a landlord, I want to upload supporting property documents — including title deeds, rental income statements, and proof of ownership — so that the underwriting team can verify my application without requiring a branch visit.
As a landlord, I want to review a consolidated summary of all my entered details and submitted documents before final submission so that I can catch errors and track my application status in real time without calling the bank.
---
The Prompt
```
